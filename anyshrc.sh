...
alias fgrea="git fetch origin && git checkout origin/main && gclient sync -D && git rebase-update --no-fetch --no-squash"
alias gb="git branch"
...
if [ -z "$JARHAR_OSX" ]; then
  ...
  export RELATIVE_CONTENT_SHELL_PATH="content_shell"
else
  export RELATIVE_CHROMIUM_PATH="Chromium.app/Contents/MacOS/Chromium"
  export RELATIVE_CONTENT_SHELL_PATH="Content\ Shell.app/Contents/MacOS/Content\ Shell"
fi
...
alias bcs="autoninja -C out/Release content_shell && out/Release/${RELATIVE_CONTENT_SHELL_PATH} --enable-experimental-web-platform-features"
alias bcsd="autoninja -C out/Debug content_shell && out/Debug/${RELATIVE_CONTENT_SHELL_PATH} --enable-experimental-web-platform-features"
...
alias lltestd="autoninja --offline -C out/Debug blink_tests content_shell && ./third_party/blink/tools/run_web_tests.py --fully-parallel -t Debug --no-retry-failures --no-show-results"
